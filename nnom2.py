...
from NNom.fully_connected_opt_weight_generation import *
from NNom.gen_config import *
import scipy.stats
import time
import warnings
...
def quantize_output(model, x_test, quantize_method='max_min', layer_offset=False, calibrate_size=100):
    ...
    def update_previous_layer_shift(layer, dec_bit):
        """if(type(layer.input) == list):
            ...
            if(not is_shift_layer(LM[iname])):
                update_previous_layer_shift(LM[iname], dec_bit)"""
        layer_input=get_layer_input_name(layer)
        if layer_input is list:
          for iname in layer_input:
            if('input' in iname):
              continue
            layer_q_list[iname][0] = dec_min
            if(not is_shift_layer(LM[iname])):
          ...
          if('input' in layer_input):
            return
          layer_q_list[layer_input][0] = dec_min
          if(not is_shift_layer(LM[layer_input])):
            update_previous_layer_shift(LM[layer_input], dec_bit)
...
def get_layer_input_name(layer):
    if type(layer.inbound_nodes[0].inbound_layers)== list:
      return [elem.name for elem in layer.inbound_nodes[0].inbound_layers]
    else:
      return layer.inbound_nodes[0].inbound_layers.name
    ...
    for curr_idx, layer in  enumerate(model.layers):
        ...
        for idx, var in enumerate(layer_weights):
            ...
            if(is_shift_layer(layer) and not is_rnn_layer(layer)):
                ...
                inp= get_layer_input_name(layer)
...
def generate_model2(model, x_test, per_channel_quant=False, name='weights.h', format='hwc', quantize_method='max_min'):
    """
    :param model:
    :param x_test:
    :param name:
    ...
    with open(name, 'a') as fp:
        ...
        for layer in L:
            ...
            # others
                ...
                inps = get_layer_input_name(layer)
                inX = ''
                for inp in inps:
